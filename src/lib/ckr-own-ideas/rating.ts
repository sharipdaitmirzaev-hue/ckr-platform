...
export function rateOwnIdea(input: {
  ...
  const hasCapital = foundKinds.has("CAPITAL");
  const officialFacts = input.components.filter(
    (c) => c.found && c.provenance.kind === "FACT" && c.provenance.trustLevel !== "general_web",
  ).length;
  ...
  if (!hasCapital || input.missing.some((m) => m.kind === "CAPITAL")) {
    return officialFacts >= 2 ? "needs_check" : "missing_data";
  }
  if (input.missing.length > 2 || officialFacts < 2) return "needs_check";
  return "promising";
}
