...
export async function loadStage3aFixtureScenario(
  service?: BusinessGraphService,
): Promise<Stage3aScenarioResult> {
  const g = service ?? createMemoryBusinessGraphService();
  await g.resetForTests();

  const capital = (
    await g.createOrUpdateNode({
      nodeType: "CAPITAL",
  ...
  const demand = (
    await g.createOrUpdateNode({
      ...
      dataQualityScore: 80,
      structuredData: {
        procurement_id: "0373100043226000123",
        nmck: 12_500_000,
      },
      sourceType: "fixture",
  ...
  await g.addAlias(capital.id, "Инвестор 30 млн", "fixture");
  await g.addAlias(demand.id, "Закупка ЕИС 0373100043226000123", "fixture");
