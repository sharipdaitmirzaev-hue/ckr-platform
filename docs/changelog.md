...
Версия платформы: `src/config/version.ts` и `/api/health`.

---

## 1.0.0 — 2026-08-10

Production release: публичный сайт без beta-бейджа, стабильная версия `1.0.0`.

### Изменения

- `platformVersion` → `1.0.0` / channel `production`
- Убран бейдж Beta из публичного сайта, кабинета, админки и операторского центра
- Legal: `/privacy`, `/terms`, согласие при регистрации
