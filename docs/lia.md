...
Цепочка, к которой стремимся:

**Идея → Проект → Возможности → Инвестиции → Эксперты → Решение**
...
1. **Помоги оформить бизнес-идею** — пошаговые вопросы → `ProjectDraft` → кнопка «Перейти к созданию проекта» (query params в форму).  
